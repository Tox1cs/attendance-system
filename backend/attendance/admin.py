...
from .models import (
    WorkShift, ShiftDayRule, Employee, RawAttendanceLog, DailyAttendanceReport, 
    OvertimeRequest, LeaveRequest, Holiday, MissionRequest, ManualLogRequest
)

class ShiftDayRuleInline(admin.TabularInline):
    model = ShiftDayRule; extra = 7; max_num = 7
...
class WorkShiftAdmin(admin.ModelAdmin):
    list_display = ('name',); inlines = [ShiftDayRuleInline]

@admin.register(Holiday)
class HolidayAdmin(admin.ModelAdmin):
    list_display = ('date', 'name'); search_fields = ('name', 'date'); list_filter = ('date',)
