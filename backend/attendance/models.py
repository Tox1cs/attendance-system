...
class ShiftDayRule(models.Model):
    ...
    def __str__(self):
        return f"{self.shift.name} - {self.get_day_of_week_display()}"
...
class DailyAttendanceReport(models.Model):
    ...
    work_shortfall_minutes = models.IntegerField(default=0)
    work_overtime_minutes = models.IntegerField(default=0)
    class Meta: unique_together = ('employee', 'date')
