  ...
  const handleMissionRequest = async (e: React.FormEvent) => {
    ...
    if (missionType === 'HOURLY') {
      if (!startTime || !endTime) {
        setMessage({ type: "error", text: "Start and End time are required for hourly missions." });
        return;
      }
    ...
    try {
      const response = await fetch("http://localhost:8000/api/mission/request/", {
        method: "POST",
      ...
      <div className="rounded-lg bg-white p-6 shadow-md">
        ...
        <form onSubmit={handleMissionRequest} className="space-y-4">
          ...
          <div>
            ...
            <input id="mission-date" type="date" value={missionDate} onChange={(e) => setMissionDate(e.target.value)}
          ...
          <div>
            ...
            <textarea id="reason" value={reason} onChange={(e) => setReason(e.target.value)}
              className="mt-1 block w-full rounded-md border-gray-300 text-gray-900 shadow-sm" rows={2}
            />
          </div>
